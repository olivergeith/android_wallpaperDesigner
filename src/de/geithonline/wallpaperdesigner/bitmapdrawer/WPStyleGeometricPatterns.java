...
public class WPStyleGeometricPatterns extends WPStylePattern {
	...
	public synchronized Bitmap drawBitmap() {
		...
		final GeometricRaster raster = new GeometricRaster(bWidth, bHeight, maxRadius);

		final int anzahlPatterns = raster.getAnzahlPattern();
		...
		for (int i = 0; i < anzahlPatterns; i++) {
			...
			final Point point = raster.getNextPoint();
			final int x = point.x;
			final int y = point.y;
