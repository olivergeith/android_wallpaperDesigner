...
public class BmpRenderer extends BaseBmpRenderer {
    ...
    public synchronized Bitmap drawBitmap(final int width, final int height) {
        ...
        final AbstractRaster raster = RasterFactory.getRaster(layout, layoutVariante, width, height, maxRadius, Settings
                        .getOverlapping());
        ...
        for (int i = 0; i < anzahlPatterns; i++) {
            ...
            if (Settings.isRandomizeColors()) {
                // pcolor = Randomizer.randomizeHue(pcolor,
                // Settings.getRandomizeColorRange());
                pcolor = Randomizer.randomizeColor(pcolor, Settings.getRandomizeColorRange(), Settings
                                .getColorRandomizingType());
            }
    ...
    private void setupDropShadow(final Bitmap refbitmap, final int dropShadowRadius, final Paint paint, final int x,
        ...
        case "Random":
            ...
            break;
        case "Opposite":
            paint.setShadowLayer(dropShadowRadius, dX, dY, getColorFromBitmap(bitmap, refbitmap, bWidth - 1 - x, bHeight
                            - 1 - y));
            break;
        case "Darker":
            paint.setShadowLayer(dropShadowRadius, dX, dY, ColorHelper.changeBrightness(pcolor, Settings
                            .getDropShadowDarkness()));
            break;
        case "Select":
