...
public class GlossyDrawer {
  ...
  public void draw(final int x, final int y, final Paint paint, final int radius, final Path path,
      final GLOSSY_REFLECTIONS_STYLE reflectionStyle, final GLOSSY_GLOW_STYLE glowStyle, final boolean ignoreIsGlossy) {
    ...
    case VERTICAL:
      ...
      break;
    case VERTICAL_WHITE:
      setupPaintShaderVerticalWhiteGlow(x, y, paint, radius);
      break;
    }
  ...
  private void setupPaintShaderVerticalGlow(final int x, final int y, final Paint paint, final int radius) {
    final int white = ColorHelper.setColorAlpha(paint.getColor(), Settings.getGlossyReflectionBrightness());
    final int transparent = Color.argb(0, 255, 255, 255);
    paint.setShadowLayer(0, 0, 0, 0);
    final int colors[] = { transparent, transparent, white, transparent, transparent };
    final float dists[] = { 0f, 0.3f, 0.5f, 0.7f, 1f };
    paint.setShader(new LinearGradient(//
        x - radius, //
        y, //
        x + radius, //
