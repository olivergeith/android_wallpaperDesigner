...
import de.geithonline.wallpaperdesigner.shapes.*;
...
import de.geithonline.wallpaperdesigner.shapes.composed.CircularArcsRotated2Path;
import de.geithonline.wallpaperdesigner.shapes.composed.CircularArcsRotatedPath;
...
public class PatternGetter {
	...
	private static Path drawCircularMaze(final int x, final int y, final int radius, String variant) {
		...
		switch (variant) {
			...
			case "V1":
			case "Maze":
				return new CircularMazePath(new PointF(x, y), radius, Settings.getCircularMazeOptions(), MazeType.V1);
			case "V2":
			case "Maze V2":
			...
			case "V5":
			case "Beam V3":
				return new CircularBeam3Path(new PointF(x, y), radius, Settings.getCircularMazeOptions());
			case "V6":
			case "Arcs rotated":
				return new CircularArcsRotatedPath(new PointF(x, y), radius, Settings.getCircularMazeOptions());
			case "V7":
			case "Arcs rotated evenly":
				return new CircularArcsRotated2Path(new PointF(x, y), radius, Settings.getCircularMazeOptions());
		}
	}
	...
	private static Path drawGeometricLong(final int x, final int y, final int radius, final String variant) {
		...
		switch (variant) {
			...
			case "Drop (Sharp)":
				return new AsymetricLongPath(new PointF(x, y), radius, radius * 6, Settings.getFilledBoolean(), ASYMETRIC_STYLE.DROP_SHARP);
			case "Drop":
			...
			case "Cross (Upsidedown)":
				return new AsymetricLongPath(new PointF(x, y), radius, radius * 6, Settings.getFilledBoolean(), ASYMETRIC_STYLE.CROSS_UPSIDE_DOWN);
			...
			case "Cross (Split - Upsidedown)":
				return new AsymetricLongPath(new PointF(x, y), radius, radius * 4, Settings.getFilledBoolean(), ASYMETRIC_STYLE.CROSS_SPLIT_UPSIDE_DOWN);
			...
			case "Cross (Slim - Upsidedown)":
				return new AsymetricLongPath(new PointF(x, y), radius, radius * 4, Settings.getFilledBoolean(), ASYMETRIC_STYLE.CROSS_SLIM_UPSIDE_DOWN);
	...
	private static Path drawWeather(final int x, final int y, final int radius, String variant) {
		...
		switch (variant) {
			...
			case "Sun with Flames (Arrows)":
				return new SunPath(12 + Settings.getAnzahlFlowerLeafs(1, 5), new PointF(x, y), radius, Settings.getFilledBoolean(),
