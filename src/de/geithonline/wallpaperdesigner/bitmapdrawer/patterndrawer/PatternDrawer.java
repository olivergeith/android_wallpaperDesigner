...
public class PatternDrawer {
    ...
    protected void drawWeather(final int x, final int y, final int radius, final String variante) {
        ...
        switch (variante) {
            ...
            case "Sun with Flames (Arrows)":
                path = new SunPath(12 + Settings.getAnzahlFlowerLeafs(1, 5), new PointF(x, y), radius, Settings.getFilledBoolean(),
    ...
    private void drawQualle2(final int x, final int y, final int radius, final String variante) {
        ...
        switch (variante) {
            default:
            case "V1":
            case "Jellyfish (Sideview)":
                // path = new QualleTopviewPath(new PointF(x, y), radius, lineTailOptions, new BubbleTailOptions());
                break;
            case "V2":
            case "Jellyfish (Topview)":
                path = new QualleTopviewPath(new PointF(x, y), radius, new TailOptionsLine(), new TailOptionsBubbles());
                break;
        }
        PathHelper.rotatePath(x, y, path, rotationDegrees);
        ...
        // inner oval
        pm.initFillPaint();
        if (path.inner != null && Settings.getFilledBoolean() == true) {
            pm.randomizeColorAccordingToSettings(pm.getInitialColor());
            pm.setColor(ColorHelper.adjustColorBrightness(pm.getCurrentColor(), 90));
    ...
    // ----------------
    // #########################################################################################
    private void drawPenguin(final int x, final int y, final int radius, final int index) {
        String variant = Settings.getSelectedPatternVariant();
        if (variant.equalsIgnoreCase("Mixed")) {
            final int nr = Randomizer.getRandomInt(1, 1);
            variant = "V" + nr;
        }
        drawPenguin(x, y, radius, variant);
    }

    private void drawPenguin(final int x, final int y, final int radius, final String variante) {
        final float rotationDegrees = rotator.getRotationDegrees(0, 360, new Point(x, y));
        PenguinPath path = null;

        switch (variante) {
            default:
            case "V1":
                path = new PenguinPath(new PointF(x, y), radius);
                break;
        }
