...
public class PatternDrawer {
	...
	private void drawCircularMaze(final int x, final int y, final int radius, final String pattern, final String variant) {
		final float rotationDegrees = rotator.getRotationDegrees(0, 360, new PointF(x, y));
	...
	private void drawNormalPattern(final int x, final int y, final int radius, final String pattern, final String variant, final float extraRotate) {
		...
		PathHelper.rotatePath(x, y, path, extraRotate + rotator.getRotationDegrees(0, 360, new PointF(x, y)));
	...
	private void drawLinePattern(final int x, final int y, final int radius, final String pattern, final String variant) {
		final Path path = PatternGetter.getPath(x, y, radius, bWidth, bHeight, pattern, variant);
		PathHelper.rotatePath(x, y, path, rotator.getRotationDegrees(0, 360, new PointF(x, y)));
	...
	private void drawBubble(final int x, final int y, final int radius) {
		...
		glossyDrawer.draw(x, y, paint, radius, path, EGlossyReflectionStyle.BIG_OVAL);
		outlineDrawer.draw(paint, radius, path);
	}

	public void drawRain(final int x, final int y, final int radius, final String pattern, final String variant) {
		final String lineStyle = Settings.getRainLineStyle();
		switch (variant) {
			default:
			case "Rain":
				if (Randomizer.getRandomBooleanTrueInPercentOfCases(Settings.getScenePercentageOfCircles())) {
					...
					// Rain
					drawLinePattern(x, y, radius, "Lines (Directed)", lineStyle);
				}
				break;
			case "Rectangle Rain":
				if (Randomizer.getRandomBooleanTrueInPercentOfCases(Settings.getScenePercentageOfCircles())) {
			...
			case "Lense":
				if (Randomizer.getRandomBooleanTrueInPercentOfCases(Settings.getScenePercentageOfCircles())) {
					// drawNormalPattern(x, y, (int) (radius * 0.65f), "Assorted Shapes", "Drop", 90); // extra 90° rotating
					drawNormalPattern(x, y, radius, "Geometrical Shapes", "Lense");
				} else {
					// Rain
					drawLinePattern(x, y, radius, "Lines (Directed)", lineStyle);
				}
	...
	private void drawScene(final int x, final int y, final int radius, final String pattern, final String variant) {
		...
		PathHelper.rotatePath(x, y, p, rotator.getRotationDegrees(0, 360, new PointF(x, y)));
		if (p instanceof TrailObjectPath) {
			final TrailObjectPath path = (TrailObjectPath) p;
			PathHelper.rotatePath(x, y, path.main, rotator.getRotationDegrees(0, 360, new PointF(x, y)));
			...
			glossyDrawer.draw(x, y, paint, radius, path.trailPath, //
					EGlossyReflectionStyle.NONE, EGlossyGlowStyle.VERTICAL, true);
		}
	}
