...
public class ShellPath extends Path {
	...
	public ShellPath(final int circles, final int abschnitte, final Point center, final float rOuter, final int variante) {
		super();
		drawSchnecke(circles, abschnitte, center, rOuter, variante);
	}

	private void drawSchnecke(final int circles, final int abschnitte, final Point center, final float rOuter, final int variante) {
		...
		for (int i = 0; i <= ecken; i++) {
			final float ri = rOuter * ((float) i / ecken) * ((float) i / ecken);
			final float ra = rOuter * (((float) i + abschnitte) / ecken) * (((float) i + abschnitte) / ecken);
			final float rc = rOuter * ((i + abschnitte * 0.7f) / ecken) * ((i + abschnitte * 0.7f) / ecken);
			// innere Punkte
