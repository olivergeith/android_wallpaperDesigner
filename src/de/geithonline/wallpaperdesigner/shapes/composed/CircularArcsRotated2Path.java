...
public class CircularArcsRotated2Path extends ComposedPath {

	private final CircularMazeOptions options;

	public CircularArcsRotated2Path(final PointF center, final float radius, final CircularMazeOptions options) {
	...
	private void draw(final PointF center, final float radius) {
		...
		for (int i = 0; i < options.anzArcs; i++) {
			...
			final float dicke = radius * Randomizer.getRandomFloat(0.8f, 1f);
