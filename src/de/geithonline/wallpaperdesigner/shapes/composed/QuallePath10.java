...
public class QuallePath10 extends ComposedPath {

    public QuallePath10(final PointF center, final float radius, final int arms, final EQualleType type) {
        switch (type) {
            default:
            case qualle:
    ...
    private void drawBubbleTail(final PointF center, final float radius, final int arms) {
        ...
        for (int i = 0; i < anz; i++) {
            ...
            final Path s =
                    new SinusObjectsPath(c, length, repeats, amplitude, maxBubbleRadius, 100, ESinusObjectsSizingType.decreasing, ESinusObjectsType.bubble);
            if (flip) {
