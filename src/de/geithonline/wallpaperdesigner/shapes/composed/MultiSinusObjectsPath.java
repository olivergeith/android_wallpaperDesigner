...
public class MultiSinusObjectsPath extends ComposedPath {
     ...
     */
    public MultiSinusObjectsPath(final PointF center, final float radius, final float minLength, final float maxLength,
            final int numberofTails, final float offset, final ESinusObjectsType objectType) {
        super();
        drawSinusBubbleTail(center, radius, minLength, maxLength, numberofTails, offset, objectType);
    }

    private void drawSinusBubbleTail(final PointF center, final float radius, final float minLength,
            final float maxLength, final int numberofTails, final float offset, final ESinusObjectsType objectType) {
        ...
        for (int i = 0; i < numberofTails; i++) {
            ...
            final Path sinus = new SinusObjectsPath(c, length, repeats, amplitude, maxBubbleRadius, 80,
                    ESinusObjectsSizingType.random, objectType);
