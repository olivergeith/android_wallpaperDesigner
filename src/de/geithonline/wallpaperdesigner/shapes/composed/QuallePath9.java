...
public class QuallePath9 extends ComposedPath {

	public QuallePath9(final PointF center, final float radius, final EQualleType type) {
		switch (type) {
		default:
		case qualle:
	...
	private void drawBubbleTail(final PointF center, final float radius) {
		...
		for (int i = 0; i < anz; i++) {
			...
			final float length = radius * Randomizer.getRandomFloat(3.5f, 5.5f);
			...
			PathHelper.rotatePath(center, s, -20 + i * 40 / anz);
			addPath(s);
		}
	}
	...
	public void drawTail(final PointF center, final float radius) {
		...
		for (int i = 0; i < anz; i++) {
			...
			final float length = radius * Randomizer.getRandomFloat(2.0f, 6.5f);
			final PointF c = new PointF();
			c.x = center.x + radius + length;
			c.y = center.y;
			...
			PathHelper.rotatePath(center, s, Randomizer.getRandomFloat(-25, 25));
			addPath(s);
		}
		...
		for (int i = 0; i < anz; i++) {
			final int repeats = 3;
			final float amplitude = radius * 0.2f;
			final float length = radius * Randomizer.getRandomFloat(1.5f, 3.5f);
			final PointF c = new PointF();
			c.x = center.x + radius + length;
			c.y = center.y;
			final Path s = new SinusPath(c, length, repeats, amplitude);
			...
			PathHelper.rotatePath(center, s, Randomizer.getRandomFloat(-40, -50));
			addPath(s);
		}
		anz = 5;
		for (int i = 0; i < anz; i++) {
