...
public class QuallePath6 extends ComposedPath {
    ...
    public QuallePath6(final PointF center, final float radius, final int arms, final EQualleType type) {
        switch (type) {
            default:
            case qualle:
    ...
    private void drawBubbleTail(final PointF center, final float radius, final int arms) {
        ...
        final int anz = 11 + arms;
        for (int i = 0; i < 15; i++) {
            final int repeats = 2; // Randomizer.getRandomInt(1, 4);
            final float amplitude = radius * 0.4f; // Randomizer.getRandomFloat(0.1f,
                                                   // 0.4f);
