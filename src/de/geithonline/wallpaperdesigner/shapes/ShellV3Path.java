...
public class ShellV3Path extends Path {

	public ShellV3Path(final int circles, final int abschnitte, final Point center, final float rOuter) {
		super();
	...
	private void drawSchnecke(final int circles, final int abschnitte, final Point center, final float rOuter) {
		...
		for (int i = 0; i <= ecken; i++) {
			...
			final float rc = (float) (rOuter * Math.pow(((i + abschnitte * 0.65f) / ecken), 3f));
			final float rci = (float) (rOuter * Math.pow(((i + abschnitte * 0.35f) / ecken), 3f));
			// innere Punkte
